...
- `.btn-primary`: pill-shaped, orange gradient fill, uppercase mono-weight label, glow shadow, scales up 1.05x on hover.
- `.btn-outline`: pill-shaped, transparent, subtle white border, brightens on hover.
...
3. **Games** (`#games`) — "Shipped & Battle-Tested"
   ...
   - Click opens **Game Detail Modal**: full-screen dark overlay, image/video carousel (arrow-key navigable), detailed description, dev challenges/postmortem text
...
All section transitions alternate background tint (`#030304` ↔ `rgba(10,12,16,0.78)`) to visually separate sections without hard borders.
...
- Reuses the same dark visual language as the public site (cards, glass panels, orange accents) so it feels like one product, not a bolted-on backend.
...
| `admin_users` | Legacy plain-text login (deprecated) | superseded by Supabase Auth in migration v2 — locked to zero access |

**Security model:** Row Level Security is enabled on every table. Public (`anon`) role gets **read-only** access; all writes require an `authenticated` Supabase Auth session (i.e., a logged-in admin). This replaced an earlier, less secure "anon key can write anything" setup.
...
## 7. Content Placeholders (default seed data)
